...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("pattern can't be "
            + "null and length must be more than 0");
        }
        if (text == null) {
        ...
        int j = 0;
        int i = 0;
        int[] ft = buildFailureTable(pattern);
        int idx;
        int count = 0;
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        Map<Character, Integer> lTable = buildLastTable(pattern);
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        int tempHash = generateHash(text, pattern.length());
        for (int i = pattern.length(); i < text.length(); i++) {
            if (patHash == tempHash) {
                boolean mismatch = false;
                for (int n = 0; n < pattern.length(); n++) {
                    if (pattern.charAt(n)
                            != text.charAt(i - pattern.length() + n)) {
                        mismatch = true;
                    }
                }
                if (!mismatch) {
            ...
            tempHash = updateHash(tempHash, pattern.length(),
                    text.charAt(i - pattern.length()), text.charAt(i));
        }
        return list;
    }
    ...
    public static int generateHash(CharSequence current, int length) {
        ...
        if (length < 0 || length == 0 || length > current.length()) {
            throw new IllegalArgumentException("Length can't be negative or"
                    + " greater than length of current");
        }
        int sum = 0;
        char c;
        for (int i = 0; i < length; i++) {
            c = current.charAt(i);
            sum += ((int) c) * pow(BASE, (length - 1 - i));
        }
        return sum;
    }
    ...
    public static int updateHash(int oldHash, int length, char oldChar,
        ...
        return (oldHash - ((int) oldChar) * pow(BASE, length - 1))
                * BASE + (int) newChar;
